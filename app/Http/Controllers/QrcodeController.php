...
class QrcodeController extends AppBaseController
{
    ...
    public function updateQrcodePath($id, $newQrcodePath)
    {
        ...
        // Actualiza el campo qrcode_path con el nuevo valor
        $qrcode->update(['qrcode_path' => $newQrcodePath]);

        Flash::success('Qrcode_path updated successfully.');
    ...
    public function imagenProducto($qrcode,$request){
        ...
        if ($request->hasFile('image')) {
            ...
            $archivo->move(public_path('images'), $fileName);

            $qrcode->update(['image' => 'images/' . $fileName]);
           
        }
    }

    /*public function updateimagen($id, $newQrcodePath)
    {
        $qrcode = $this->qrcodeRepository->find($id);
